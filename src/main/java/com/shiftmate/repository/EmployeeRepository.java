...
/**
 * Data access layer for {@link Employee} entities. Custom queries use JOIN FETCH
 * to avoid N+1 problems when role data must be available on the returned objects.
 */
@Repository
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
     ...
     *
     * @param restaurantId the restaurant's ID
     * @param isActive     {@code true} for active employees, {@code false} for inactive
     ...
     * in a single query. Used by the schedule builder to populate assignment dropdowns.
     *
     * @param restaurantId the restaurant's ID
     * @return active employees ordered by last name then first name, with roles populated
     */
    @Query("""
